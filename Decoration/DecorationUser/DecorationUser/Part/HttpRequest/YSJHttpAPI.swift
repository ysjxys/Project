...
struct YSJHttpAPI {
    ...
    static func send<T: YSJHttpRequest> (request: T, successHandle: @escaping (T.Model?) -> Void, unSuccessHandle:@escaping (Int?, String?) -> Void, failHandle: ((Error?) -> Void)?, netErrorHandle: (() -> Void)? = nil ){
        ...
        Alamofire.request(url, method: method, parameters: param, encoding: encoding, headers: header).responseJSON { (response) in
            ...
            if let valueDic = response.value as? [String: Any] {
                if valueDic["resultCode"] as? Int ?? 0 == 2000 {
                    successHandle(T.Model.deserialize(from: valueDic["data"] as? [String: Any]))
                } else {
                    unSuccessHandle(valueDic["resultCode"] as? Int, valueDic["resultMsg"] as? String)
                }
            }
